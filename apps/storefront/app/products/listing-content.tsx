...
export function ProductListingContent() {
  ...
  // Update document title for category pages (client-side SEO fallback)
  React.useEffect(() => {
    if (activeCategory) {
      const catName = categories.find((c) => c.slug === activeCategory)?.name || activeCategory;
      document.title = `${catName} | Ringside Sports`;
      const metaDesc = document.querySelector('meta[name="description"]');
      if (metaDesc) {
        metaDesc.setAttribute(
          "content",
          `Shop ${catName} at Ringside Sports — Australia's factory outlet. Premium combat sports equipment at unbeatable prices. Fast shipping nationwide.`
        );
      }
    }
  }, [activeCategory]);
